...
namespace DIPSky.Services.Directory
{
    /// <summary>
    ...
    public partial class MeasureService : IMeasureService
    {
        ...
        /// Key for caching
        /// </summary>
        private const string MEASUREDIMENSIONS_ALL_KEY = "DIPSky.measuredimension.all";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        /// {0} : dimension ID
        /// </remarks>
        private const string MEASUREDIMENSIONS_BY_ID_KEY = "DIPSky.measuredimension.id-{0}";
        /// <summary>
        /// Key for caching
        /// </summary>
        private const string MEASUREWEIGHTS_ALL_KEY = "DIPSky.measureweight.all";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        /// {0} : weight ID
        /// </remarks>
        private const string MEASUREWEIGHTS_BY_ID_KEY = "DIPSky.measureweight.id-{0}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string MEASUREDIMENSIONS_PATTERN_KEY = "DIPSky.measuredimension.";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string MEASUREWEIGHTS_PATTERN_KEY = "DIPSky.measureweight.";
            ...
            MeasureDimension sourceMeasureDimension)
        {
            ...
            if (result != decimal.Zero && sourceMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new DipSkyException(string.Format("Exchange ratio not set for dimension [{0}]", sourceMeasureDimension.Name));
            ...
            MeasureDimension targetMeasureDimension)
        {
            ...
            if (result != decimal.Zero && targetMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new DipSkyException(string.Format("Exchange ratio not set for dimension [{0}]", targetMeasureDimension.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
        ...
        public virtual decimal ConvertToPrimaryMeasureWeight(decimal quantity, MeasureWeight sourceMeasureWeight)
        {
            ...
            if (result != decimal.Zero && sourceMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new DipSkyException(string.Format("Exchange ratio not set for weight [{0}]", sourceMeasureWeight.Name));
                result = result / exchangeRatio;
            }
            return result;
        }
            ...
            MeasureWeight targetMeasureWeight)
        {
            ...
            if (result != decimal.Zero && targetMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new DipSkyException(string.Format("Exchange ratio not set for weight [{0}]", targetMeasureWeight.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
