    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (customer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                    throw new DipSkyException("Anonymous checkout is not allowed");
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    if (customer.BillingAddress == null)
                        throw new DipSkyException("Billing address is not provided");

                    if (!CommonHelper.IsValidEmail(customer.BillingAddress.Email))
                        throw new DipSkyException("Email is not valid");
                    ...
                    if (billingAddress.Country != null && !billingAddress.Country.AllowsBilling)
                        throw new DipSkyException(string.Format("Country '{0}' is not allowed for billing", billingAddress.Country.Name));
                }
                else
                {
                    if (initialOrder.BillingAddress == null)
                        throw new DipSkyException("Billing address is not available");
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (cart.Count == 0)
                        throw new DipSkyException("Cart is empty");
                    ...
                    if (warnings.Count > 0)
                    {
                        ...
                        throw new DipSkyException(warningsSb.ToString());
                    }
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (!minOrderSubtotalAmountOk)
                    {
                        ...
                        throw new DipSkyException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"), _priceFormatter.FormatPrice(minOrderSubtotalAmount, true, false)));
                    }
                    bool minOrderTotalAmountOk = ValidateMinOrderTotalAmount(cart);
                    if (!minOrderTotalAmountOk)
                    {
                        decimal minOrderTotalAmount = _currencyService.ConvertFromPrimaryStoreCurrency(_orderSettings.MinOrderTotalAmount, _workContext.WorkingCurrency);
                        throw new DipSkyException(string.Format(_localizationService.GetResource("Checkout.MinOrderTotalAmount"), _priceFormatter.FormatPrice(minOrderTotalAmount, true, false)));
                    }
                }
                ...
                if (shoppingCartRequiresShipping)
                {
                    if (!processPaymentRequest.IsRecurringPayment)
                    {
                        if (customer.ShippingAddress == null)
                            throw new DipSkyException("Shipping address is not provided");
                        ...
                        if (shippingAddress.Country != null && !shippingAddress.Country.AllowsShipping)
                            throw new DipSkyException(string.Format("Country '{0}' is not allowed for shipping", shippingAddress.Country.Name));
                    ...
                    else
                    {
                        if (initialOrder.ShippingAddress == null)
                            throw new DipSkyException("Shipping address is not available");
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                        throw new DipSkyException("Shipping total couldn't be calculated");
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (!orderTotal.HasValue)
                        throw new DipSkyException("Order total couldn't be calculated");
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    if (paymentMethod == null)
                        throw new DipSkyException("Payment method couldn't be loaded");

                    //ensure that payment method is active
                    if (!paymentMethod.IsPaymentMethodActive(_paymentSettings))
                        throw new DipSkyException("Payment method is not active");
                }
                else
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (isRecurringShoppingCart)
                    {
                        ...
                        if (!string.IsNullOrEmpty(recurringCyclesError))
                            throw new DipSkyException(recurringCyclesError);
                ...
                if (!skipPaymentWorkflow)
                {
                        ...
                        if (isRecurringShoppingCart)
                        {
                            ...
                            switch (recurringPaymentType)
                            {
                                case RecurringPaymentType.NotSupported:
                                    throw new DipSkyException("Recurring payments are not supported by selected payment method");
                                case RecurringPaymentType.Manual:
                                case RecurringPaymentType.Automatic:
                                    processPaymentResult = _paymentService.ProcessRecurringPayment(processPaymentRequest);
                                    break;
                                default:
                                    throw new DipSkyException("Not supported recurring payment type");
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            throw new DipSkyException("No recurring products");
                        }
                    }
                }
                else
                ...
                if (processPaymentResult == null)
                    throw new DipSkyException("processPaymentResult is not available");
        ...
        public virtual void ProcessNextRecurringPayment(RecurringPayment recurringPayment)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new DipSkyException("Recurring payment is not active");

                var initialOrder = recurringPayment.InitialOrder;
                if (initialOrder == null)
                    throw new DipSkyException("Initial order could not be loaded");

                var customer = initialOrder.Customer;
                if (customer == null)
                    throw new DipSkyException("Customer could not be loaded");

                var nextPaymentDate = recurringPayment.NextPaymentDate;
                if (!nextPaymentDate.HasValue)
                    throw new DipSkyException("Next payment date could not be calculated");
                ...
                if (result.Success)
                {
                    if (result.PlacedOrder == null)
                        throw new DipSkyException("Placed order could not be loaded");
                ...
                else
                {
                    ...
                    throw new DipSkyException(error);
                }
            }
            catch (Exception exc)
            {
        ...
        public virtual void CancelOrder(Order order, bool notifyCustomer)
        {
            ...
            if (!CanCancelOrder(order))
                throw new DipSkyException("Cannot do cancel for order.");
        ...
        public virtual IList<string> Capture(Order order)
        {
            ...
            if (!CanCapture(order))
                throw new DipSkyException("Cannot do capture for order.");
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new DipSkyException("You can't mark this order as paid");
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            if (!CanRefund(order))
                throw new DipSkyException("Cannot do refund for order.");
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new DipSkyException("You can't refund this order");
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new DipSkyException("Cannot do partial refund for order.");
        ...
        public virtual void PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new DipSkyException("You can't partially refund (offline) this order");
        ...
        public virtual IList<string> Void(Order order)
        {
            ...
            if (!CanVoid(order))
                throw new DipSkyException("Cannot do void for order.");
        ...
        public virtual void VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new DipSkyException("You can't void this order");
