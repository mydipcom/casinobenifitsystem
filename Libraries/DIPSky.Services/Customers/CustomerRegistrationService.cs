    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            if (registeredRole == null)
                throw new DipSkyException("'Registered' role could not be loaded");
            ...
            if (!request.Customer.IsGuest())
            {
                ...
                request.Customer.Addresses.Add(address);
                request.Customer.BillingAddress = address;
                request.Customer.ShippingAddress = address;
            }
        ...
        public virtual void SetEmail(Customer customer, string newEmail)
        {
            ...
            if (!CommonHelper.IsValidEmail(newEmail))
                throw new DipSkyException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new DipSkyException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new DipSkyException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new DipSkyException("Usernames are disabled");

            if (!_customerSettings.AllowUsersToChangeUsernames)
                throw new DipSkyException("Changing usernames is not allowed");

            newUsername = newUsername.Trim();

            if (newUsername.Length > 100)
                throw new DipSkyException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new DipSkyException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
