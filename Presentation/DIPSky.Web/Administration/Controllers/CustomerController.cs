...
using DIPSky.Admin.Models.ShoppingCart;
using DIPSky.Core;
        ...
        public ActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                if (String.IsNullOrWhiteSpace(customer.Email))
                    throw new DipSkyException("Customer email is empty");
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new DipSkyException("Customer email is not valid");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new DipSkyException("Email subject is empty");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Body))
                    throw new DipSkyException("Email body is empty");

                var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId);
                if (emailAccount == null)
                    emailAccount = _emailAccountService.GetAllEmailAccounts().FirstOrDefault();
                if (emailAccount == null)
                    throw new DipSkyException("Email account can't be loaded");
        ...
        public ActionResult SendPm(CustomerModel model)
        {
            ...
            try
            {
                if (!_forumSettings.AllowPrivateMessages)
                    throw new DipSkyException("Private messages are disabled");
                if (customer.IsGuest())
                    throw new DipSkyException("Customer should be registered");
                if (String.IsNullOrWhiteSpace(model.SendPm.Subject))
                    throw new DipSkyException("PM subject is empty");
                if (String.IsNullOrWhiteSpace(model.SendPm.Message))
                    throw new DipSkyException("PM message is empty");
