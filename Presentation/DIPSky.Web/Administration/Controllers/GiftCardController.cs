...
{
    [AdminAuthorize]
    public partial class GiftCardController : BaseDIPSkyController
    {
        #region Fields
        ...
        public ActionResult NotifyRecipient(GiftCardModel model)
        {
            ...
            try
            {
                if (!CommonHelper.IsValidEmail(giftCard.RecipientEmail))
                    throw new DipSkyException("Recipient email is not valid");
                if (!CommonHelper.IsValidEmail(giftCard.SenderEmail))
                    throw new DipSkyException("Sender email is not valid");
