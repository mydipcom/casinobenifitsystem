    ...
    public class RegisterValidator : AbstractValidator<RegisterModel>
    {
        public RegisterValidator(ILocalizationService localizationService, CustomerSettings customerSettings)
        {
            ...
            if (customerSettings.UsernamesEnabled)
            {
                ...
                RuleFor(x => x.Username).Matches(@"\d{8}", RegexOptions.IgnoreCase).WithMessage(localizationService.GetResource("Common.WrongUsername"));  
            }
...
/*            RuleFor(x => x.FirstName).NotEmpty().WithMessage(localizationService.GetResource("Account.Fields.FirstName.Required"));
            ...
            if (customerSettings.PhoneRequired && customerSettings.PhoneEnabled)
            {
                ...
                RuleFor(x => x.Phone).Matches(@"^(\d|-)+$", RegexOptions.IgnoreCase).WithMessage(localizationService.GetResource("Common.WrongPhone"));   
            }
