...
namespace DIPSky.Web.Infrastructure.Installation
{
    /// <summary>
    ...
    {
        private const string LanguageCookieName = "DIPSky.installation.lang";
                ...
                foreach (var filePath in Directory.EnumerateFiles(webHelper.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
                {
                    ...
                    foreach (XmlNode resNode in xmlDocument.SelectNodes(@"//Language/LocaleResource"))
                    {
                        ...
                        if (resNameAttribute == null)
                            throw new DipSkyException("All installation resources must have an attribute Name=\"Value\".");
                        var resourceName = resNameAttribute.Value.Trim();
                        if (string.IsNullOrEmpty(resourceName))
                            throw new DipSkyException("All installation resource attributes 'Name' must have a value.'");

                        if (resValueNode == null)
                            throw new DipSkyException("All installation resources must have an element \"Value\".");
