...
using DIPSky.Plugin.Shipping.USPS.Domain;
    ...
    public class USPSComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        private string CreateRequest(string username, string password, GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            if (usedMeasureWeight == null)
                throw new DipSkyException(string.Format("USPS shipping service. Could not load \"{0}\" measure weight", MEASUREWEIGHTSYSTEMKEYWORD));
            ...
            if (baseusedMeasureWeight == null)
                throw new DipSkyException("Primary weight can't be loaded");
            
            var usedMeasureDimension = _measureService.GetMeasureDimensionBySystemKeyword(MEASUREDIMENSIONSYSTEMKEYWORD);
            if (usedMeasureDimension == null)
                throw new DipSkyException(string.Format("USPS shipping service. Could not load \"{0}\" measure dimension", MEASUREDIMENSIONSYSTEMKEYWORD));

            var baseusedMeasureDimension = _measureService.GetMeasureDimensionById(_measureSettings.BaseDimensionId);
            if (usedMeasureDimension == null)
                throw new DipSkyException("Primary dimension can't be loaded");
        ...
        public void GetConfigurationRoute(out string actionName, out string controllerName, out RouteValueDictionary routeValues)
        {
            ...
            routeValues = new RouteValueDictionary() { { "Namespaces", "DIPSky.Plugin.Shipping.USPS.Controllers" }, { "area", null } };
        }
